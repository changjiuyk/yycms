...
$result = mysql_query('select * from yycms_hytc where tc_rmb="'.$_GET['money'].'"');
$roww = mysql_fetch_array($result); 
$resultt = mysql_query('select * from yycms_user where b_user="'.$_COOKIE['user_name'].'"');
$rowcx = mysql_fetch_array($resultt);
$typee=$rowcx['b_type'];
//判定时间是否到期
if($typee < date('Y-m-d h:i:s', time())){
$b_type = time() + 86400*$roww['tc_type'];//到期增加天数
}
else{
$b_type = strtotime($typee) + 86400*$roww['tc_type'];//没到期增加天数
}
?>
<!DOCTYPE HTML>
<html>
